<template>
  <v-container fluid class="fill-height">
      ...
      <v-col cols="12" md="4">
        ...
        >
          ...
          <v-date-picker range v-model="dateRange" no-title>
            ...
            <v-btn text color="primary" @click="getSample(dateRangeText1, dateRangeText2)">OK</v-btn>
          </v-date-picker>
        </v-menu>
      </v-col>
    </v-row>
      ...
      <template v-if="sample.length > 0">
        <v-row key="metrics">
          ...
          <v-col cols="12" md="4" sm="6" v-for="item in procTotalsProc" :key="item.label">
            <v-card @click="rangeDetailShow(item.label)">
              <v-card-title>
    ...
    <v-dialog v-model="detailPop" max-width="750" scrollable>
      <v-card>
        ...
        <v-card-subtitle :style="'background:' + selProcColor" class="white--text">{{ selProcDtl }}</v-card-subtitle>
              ...
              <v-container class="fill-height">
                <v-row justify="center" align="center">
                  <v-col class="text-center">
                    <v-progress-circular width="10" indeterminate size="100" :color="selProcColor"></v-progress-circular>
    ...
    <v-dialog v-model="rangeDetailPop" max-width="750" scrollable>
      <v-card>
        <v-card-title :style="'background:' + selProcColor" class="white--text">
  ...
  watch: {
    ...
    rangeDetailPop() {
      ...
      if (!self.rangeDetailPop) {
        self.rangeDetail.type = "";
        self.rangeDetail.dataSource.dataset = [];
      }
    },
    detailPop() {
      var self = this;
  ...
  methods: {
        ...
        self.detail.dataSource.data = _.sortBy(
          _.uniqWith(
            self.sample
              .filter(function (a) {
                return (
                  new Date(a.ADD_DATE_TIME).toISOString().substr(0, 10) == selDt
                );
              })
              .map(function (n) {
                return {
    ...
    exportCSV: function () {
      ...
      var todayDt =
        ...
        dt.getFullYear();
      var arrData =
        typeof self.sample != "object" ? JSON.parse(self.sample) : self.sample;
      var CSV = "";
      if (ShowLabel) {
        var row = "";
